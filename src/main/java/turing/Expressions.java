...
/**
 * A class for experimenting with expression trees.
 *
 * <p>This class includes a nested abstract class and several subclasses that represent nodes in an
 * expression tree. It also includes several methods that work with these classes.
 */
public class Expressions {
  ...
  public static void main(String[] args) {
    ...
    for (double[] dt1 : dt) {
      System.out.println("   x = " + dt1[0] + "; y = " + dt1[1]);
    }
  }

  /**
   * Given an ExpNode that is the root of an expression tree, this method makes a full copy of the
   * tree.
   *
   * <p>The tree that is returned is constructed entirely of freshly made nodes. (That is, there are
   * no pointers back into the old copy.)
   */
  static ExpNode copy(ExpNode root) {
    if (root instanceof ConstNode) return new ConstNode(((ConstNode) root).number);
    else if (root instanceof VariableNode) return new VariableNode();
    else {
  ...
  /**
   * Returns an n-by-2 array containing sample input/output pairs.
   *
   * <p>If the return value is called data, then data[i][0] is the i-th input (or x) value and
   * data[i][1] is the corresponding output (or y) value. (This method is currently unused, except
   * to test it.)
   */
  ...
  /**
   * An abstract class that represents a general node in an expression tree.
   *
   * <p>Every such node must be able to compute its own value at a given input value, x. Also, nodes
   * should override the standard toString() method to return a fully parameterized string
  ...
  static class ConstNode extends ExpNode {
    ...
    public String toString() {
      if (number < 0) return "(" + number + ")"; // add parentheses around negative number
      else return "" + number; // just convert the number to a string
    }
  }
  ...
  /** A node in an expression tree that represents one of the binary operators +, -, *, or /. */
