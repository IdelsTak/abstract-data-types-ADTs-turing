...
/**
 * A test program for the Tape class that creates a window containing a graphical representation of
 * the tape with an arrow pointing to the current cell.
 *
 * <p>There are buttons for moving the current cell to the left and to the right.
 *
 * <p>A text-input box shows the content of the current cell.
 *
 * <p>This box can be edited, and there is a "Set" button that copies the contents of the cell
 * (actually just the first character) to the current cell.
 */
public class TestTapeGUI extends JPanel {
  ...
  private final JButton setContentButton;
  private final JTextField contentInput;
  ...
  private class TapePanel extends JPanel {
    ...
    public void paintComponent(Graphics g) {
      ...
      for (int i = 0; i <= ct; i++) tape.moveLeft();
      for (int i = 1; i <= ct; i++) {
        g.drawLine(mid - 30 * i - 15, 30, mid - 30 * i - 15, 59);
        tape.moveLeft();
        g.drawString("" + tape.getContent(), mid - 30 * i - 8, 53);
      }
      for (int i = 1; i <= ct; i++) tape.moveRight();
    }
  }
  ...
  private class ButtonListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evt) {
      if (evt.getSource() == moveLeftButton) tape.moveLeft();
      else if (evt.getSource() == moveRightButton) tape.moveRight();
      else {
        String content = contentInput.getText();
        if (content.length() == 0) tape.setContent(' ');
        else tape.setContent(content.charAt(0));
      }
